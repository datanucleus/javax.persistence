 ...
 * When used to override a mapping defined by a mapped superclass, <code>AssociationOverride</code> is applied
 * to the entity class.
 * <p>
 ...
 * <code>AssociationOverride</code> is applied to the field or property containing the embeddable.
 * <p>
 * When <code>AssociationOverride</code> is used to override a relationship mapping from an embeddable class,
 * the <code>name</code> element specifies the referencing relationship field or property within the
 * embeddable class. To override mappings at multiple levels of embedding, a dot (".") notation syntax must be
 * used in the <code>name</code> element to indicate an attribute within an embedded attribute. The value of
 * each identifier used with the dot notation is the name of the respective embedded field or property.
 * <p>
 * When <code>AssociationOverride</code> is applied to override the mappings of an embeddable class used as a
 * map value, "<code>value.</code>" must be used to prefix the name of the attribute within the embeddable
 * class that is being overridden in order to specify it as part of the map value.
 * <p>
 * If the relationship mapping is a foreign key mapping, the <code>joinColumns</code> element is used. If the
 * relationship mapping uses a join table, the <code>joinTable</code> element must be specified to override
...
public @interface AssociationOverride
{
    ...
    /**
     * The join column(s) being mapped to the persistent attribute(s). The <code>joinColumns</code> elements
     * must be specified if a foreign key mapping is used in the overriding of the mapping of the
     * relationship. The <code>joinColumns</code> element must not be specified if a join table is used in the
    ...
    /**
     * The join table that maps the relationship. The <code>joinTable</code> element must be specified if a
     * join table is used in the overriding of the mapping of the relationship. The <code>joinTable</code>
